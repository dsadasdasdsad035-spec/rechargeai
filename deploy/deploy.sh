...
echo "==> 重启服务..."
SSHPASS="$SSHPASS" sshpass -e ssh -o StrictHostKeyChecking=no "$SERVER" "export DOCKER_HOST=unix:///run/podman/podman.sock && cd $REMOTE_DIR && docker-compose -f docker-compose.prod.yml up -d --build"
